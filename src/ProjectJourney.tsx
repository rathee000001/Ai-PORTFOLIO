...
export default function ProjectJourney({project,projects,paused,reduced,onToggleMotion}:{project:Project;projects:Project[];paused:boolean;reduced:boolean;onToggleMotion:()=>void}){
 ...
 <main ref={content} className="journey-content">{project.chapters.map((chapter,index)=><section key={chapter.id} id={`section-${index}`} ref={el=>{sections.current[index]=el;}} className={`journey-section ${chapter.id==='skills-tools'?'journey-finale':''}`} aria-label={`${index+1}. ${chapter.title}`}><div className="journey-copy">
