...
export default function ProjectJourney({project,projects,paused,reduced,onToggleMotion}:{project:Project;projects:Project[];paused:boolean;reduced:boolean;onToggleMotion:()=>void}){
 const sections=useRef<(HTMLElement|null)[]>([]);const [active,setActive]=useState(0);const motionOff=paused||reduced;
 ...
 <SceneWorld project={project} active={active} paused={motionOff}/><div className="journey-veil" aria-hidden="true"/><Header paused={paused} reduced={reduced} onToggleMotion={onToggleMotion}/>
 <main className="journey-content">{project.chapters.map((chapter,index)=><section key={chapter.id} id={`section-${index}`} ref={el=>{sections.current[index]=el;}} className={`journey-section ${chapter.id==='skills-tools'?'journey-finale':''}`} aria-label={`${index+1}. ${chapter.title}`}><div className="journey-copy">
 ...
 </div><WorldArtifacts project={project} active={index} inline/></section>)}</main>
