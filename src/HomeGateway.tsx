...
import {homeDirections} from './liveSceneDirections';
...
export default function HomeGateway(props:{projects:Project[];paused:boolean;reduced:boolean;onToggleMotion:()=>void}){
 ...
 const impact=()=>{scene.current?.animate([{translate:"0 0"},{translate:"0 4px"},{translate:"-2px -2px"},{translate:"2px 1px"},{translate:"0 0"}],{duration:310,easing:"ease-out"});};
 const [progress,setProgress]=useState(0),[size,setSize]=useState(()=>({w:innerWidth,h:innerHeight}));
 ...
 return <div className={`home-gateway ${motionOff?'entry-still':''}`} style={{'--entry-zoom':motionOff?1:cameraZoom} as CSSProperties}><div ref={scene} className="entry-scene"><>{sceneAssets.map((asset,i)=><div key={asset} className="entry-art" style={{backgroundImage:`url("${asset}")`,opacity:motionOff?(i===active?1:0):i<=Math.floor(progress)?1:i===Math.ceil(progress)?progress%1:0}}/>)}</><nav className="entry-world-links" aria-label="Clickable category worlds" inert={progress>.3} style={{opacity:Math.max(0,1-progress*4),pointerEvents:progress>.3?'none':undefined}}>{worldCategories.map(c=>{const p=positions[c.slug];return <a key={c.slug} href={worldDestination(c,props.projects)} aria-label={`Enter ${c.name} world`} style={{left:`${50+(p.x-.5)*100*sx}%`,top:`${50+(p.y-.5)*100*sy}%`,width:`${p.rx*200*sx}%`,height:`${p.ry*200*sy}%`,'--portal-color':c.color,'--portal-art':'url('+c.asset+')','--portal-image-size':`${size.w*sx}px ${size.h*sy}px`,'--portal-image-position':`${-(p.x-p.rx)*size.w*sx}px ${-(p.y-p.ry)*size.h*sy}px`} as CSSProperties}><i className="portal-face" aria-hidden="true"/><span>{c.name}<ArrowUpRight size={12}/></span></a>;})}</nav><LivingScene direction={homeDirections[active]} paused={motionOff} accent="#eacda1"/></div><Header {...props} showHomeCharacter={active>0}/><main className="entry-copy" key={active}><p className="entry-kicker">{String(active+1).padStart(2,'0')} / {chapter.name.toUpperCase()} · PRAVEEN RATHEE</p><h1>{chapter.title}</h1><p className="entry-body">{chapter.body}</p><p className="entry-help">{chapter.note}</p><div className="entry-actions">{active===0?<><EvidenceLink url="https://www.linkedin.com/in/praveen-rathee-8b028030b/" label="LinkedIn"/><EvidenceLink url="https://github.com/rathee000001" label="GitHub"/></>:null}</div></main><HomeGuide active={active} still={motionOff} onLand={impact}/><nav className="entry-sections" aria-label="Home sections">{chapters.map((c,i)=><button key={c.name} aria-label={`${String(i+1).padStart(2,'0')} ${c.name}`} onClick={()=>go(i)} aria-current={active===i?'step':undefined}>{String(i+1).padStart(2,'0')}<span>{c.name}</span></button>)}</nav><div className="entry-bottom"><button disabled={active===0} onClick={()=>go(active-1)}><ArrowLeft size={15}/> Back</button><span>Home · {chapter.name} · {active+1}/4</span><button disabled={active===3} onClick={()=>go(active+1)}>Next <ArrowRight size={15}/></button></div><div style={{height:'400svh'}} aria-hidden="true"/></div>;
}
